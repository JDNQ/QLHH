...
export default function ProfilePage() {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await api.patch("/users/profile/update", { avatar: url });

      // Dùng url từ upload, KHÔNG dùng res.data.data
  ...
  return (
    <div className="space-y-6 max-w-2xl">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400 mt-0.5">
              {avatarUploading
                ? "Đang upload..."
                : "Click vào icon camera để đổi ảnh"}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Profile form */}
      <div className="bg-white rounded-xl p-5 border border-neutral-100 shadow-card">
        <h2 className="text-sm font-semibold text-neutral-800 mb-4 pb-3 border-b border-neutral-100">
          Thông tin cá nhân
        </h2>
        <form
