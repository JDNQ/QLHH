...
async function main() {
  ...
  const post1 = await prisma.post.upsert({
    ...
    create: {
      ...
      content: '<p>Máy mới 99%, còn bảo hành 10 tháng. Đầy đủ hộp phụ kiện.</p><p>Liên hệ để được tư vấn thêm.</p>',
      price: '28000000',
      priceUnit: 'VNĐ',
  ...
  await prisma.postCategory.upsert({
    where: { postId_categoryId: { postId: post1.id, categoryId: 'cat-phones' } },
    update: {},
  ...
  const post2 = await prisma.post.upsert({
    ...
    create: {
      ...
      content: '<p>Xe đi được 8.000km, máy êm không lạnh lẽo, không đâm đụng, còn bảo hiểm xe máy.</p>',
      price: '17500000',
      priceUnit: 'VNĐ',
      location: 'Hà Nội',
      status: 'APPROVED',
  ...
  await prisma.postCategory.upsert({
    where: { postId_categoryId: { postId: post2.id, categoryId: 'cat-motorbikes' } },
    update: {},
  ...
  const post3 = await prisma.post.upsert({
    ...
    create: {
      ...
      content: '<p>Phòng rộng 25m2, có gác lửng, toilet riêng, ban công, cửa sổ thoáng mát. Giá 3.5 triệu/tháng bao điện nước.</p>',
      price: '3500000',
      priceUnit: 'VNĐ/tháng',
  ...
  await prisma.postCategory.upsert({
    where: { postId_categoryId: { postId: post3.id, categoryId: 'cat-realestate' } },
    update: {},
